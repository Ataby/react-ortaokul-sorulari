...
function Soru5sosyal() {
  ...
  const handleOptionChange = (index, option) => {
    ...
    if (option === 'B' && index >= 9 && index <= 22) {
      const newImageBackgrounds = [...imageBackgrounds];
      newImageBackgrounds[index] = 'green';
      correctAnswer();
      setpuan( puan => puan+1);
      setImageBackgrounds(newImageBackgrounds);
    } else if (option !== 'B' && index >= 9 && index <= 22) {
      const newImageBackgrounds = [...imageBackgrounds];
      newImageBackgrounds[index] = 'red';
      setpuan( puan => puan-1);
      wrongAnswer();
    ...
    if (option === 'C' && index >= 23 && index <= 32) {
      const newImageBackgrounds = [...imageBackgrounds];
      newImageBackgrounds[index] = 'green';
      correctAnswer();
      setpuan( puan => puan+1);
      setImageBackgrounds(newImageBackgrounds);
    } else if (option !== 'C' && index >= 23 && index <= 32) {
      const newImageBackgrounds = [...imageBackgrounds];
      newImageBackgrounds[index] = 'red';
      wrongAnswer();
      setpuan( puan => puan-1);
    ...
    if (option === 'D' && index >= 33 && index <= 41) {
      const newImageBackgrounds = [...imageBackgrounds];
      newImageBackgrounds[index] = 'green';
      correctAnswer();
      setpuan( puan => puan+1);
      setImageBackgrounds(newImageBackgrounds);
    } else if (option !== 'D' && index >= 33 && index <= 41) {
      const newImageBackgrounds = [...imageBackgrounds];
      newImageBackgrounds[index] = 'red';
      wrongAnswer();
      setpuan( puan => puan-1);
  ...
  const Counter = () => {
   ...
   return (
    <div style={{margin:"auto 1rem"}}>
        <div style={{margin:"auto 1rem"}} >
        <h2 style={{backgroundColor:"purple", padding:"1rem" ,color:"white", margin:"auto"}} >5.SINIF SOSYAL BİLGİLER SORULAR</h2>
        </div>
      
      <div style={{margin:"auto 1rem", border:"2px solid purple"}} >
        <div className="flex-column d-flex align-items-center  ">
          <div style={{padding:".5rem",border:"3px solid grey",width:"fit-content",margin:"1rem auto",borderRadius:"8px",boxShadow:"0px 0px 5px",alignItems:"center" , backgroundColor: imageBackgrounds[randomIndex] }}>
            <img  src={`/images/sosyal5images/image${randomIndex + 1}.png`} alt={`Random Image`} />
            <div >
          ...
          <Counter  />
          <p style={{color:"white",fontSize:"2rem", marginLeft:"auto", padding:"1rem",backgroundColor:"purple"}}> Puan Durumu: {puan}</p>
          
          {((count<=0) )&&<p style={{color:"white",fontSize:"2rem", marginLeft:"auto", padding:"1rem",backgroundColor:"purple"}}> {puan} puan kazandın. Tebrikler !</p>}
  
          {((limitedQues<6) && (count>0) )&&( <button style={{backgroundColor:"purple",color:"white",padding:".5rem"}}   onClick={handleClick}>SONRAKİ SORU</button>)}
        </div>
        <button onClick={() => window.location.reload(false)} style={{backgroundColor:"purple",color:"white",padding:".5rem"}} >SAYFAYI YENİLE</button>  
  </div>
    </div>
  );
}

export default Soru5sosyal;
