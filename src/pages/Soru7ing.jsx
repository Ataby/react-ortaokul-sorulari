...
function Soru7ing() {
  ...
  const images = Array.from({ length: 32 }, ( _, index) => ({
    path: `/images/ing7images/image${index + 1}.png`,
    number: index + 1
  }));
  ...
  const handleOptionChange = (index, option) => {
    ...
    if (option === 'B' && index >= 12 && index <= 15) {
      const newImageBackgrounds = [...imageBackgrounds];
      newImageBackgrounds[index] = 'green';
      correctAnswer();
      setpuan( puan => puan+1);
      setImageBackgrounds(newImageBackgrounds);
    } else if (option !== 'B' && index >= 12 && index <= 15) {
      const newImageBackgrounds = [...imageBackgrounds];
      newImageBackgrounds[index] = 'red';
      setpuan( puan => puan-1);
      wrongAnswer();
    ...
    if (option === 'D' && index >= 24 && index <= 32) {
      const newImageBackgrounds = [...imageBackgrounds];
      newImageBackgrounds[index] = 'green';
      correctAnswer();
      setpuan( puan => puan+1);
      setImageBackgrounds(newImageBackgrounds);
    } else if (option !== 'D' && index >= 24 && index <= 32) {
      const newImageBackgrounds = [...imageBackgrounds];
      newImageBackgrounds[index] = 'red';
      wrongAnswer();
      setpuan( puan => puan-1);
  ...
  const Counter = () => {
   ...
   return (
    <div style={{margin:"auto 1rem"}}>
        <div style={{margin:"auto 1rem"}} >
        <h2 style={{backgroundColor:"green", padding:"1rem" ,color:"white", margin:"auto"}} >7.SINIF İNGİLİZCE SORULAR</h2>
        </div>
      ...
      <div style={{margin:"auto 1rem", border:"2px solid green"}} >
        <div className="flex-column d-flex align-items-center  ">
          <div style={{padding:".5rem",border:"3px solid grey",width:"fit-content",margin:"1rem auto",borderRadius:"8px",boxShadow:"0px 0px 5px",alignItems:"center" , backgroundColor: imageBackgrounds[randomIndex] }}>
            <img  src={`/images/ing7images/image${randomIndex + 1}.png`} alt={`Random Image`} />
            <div >
