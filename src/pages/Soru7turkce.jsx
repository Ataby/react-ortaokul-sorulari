...
function Soru7turkce() {
  // Resimlerin yollarını ve numaralarını içeren bir dizi (örneğin, 32 resim)
  const images = Array.from({ length: 39 }, ( _, index) => ({
    path: `/images/turkce7images/image${index + 1}.png`,
    number: index + 1
  }));
  ...
  const handleOptionChange = (index, option) => {
    ...
    if (option === 'B' && index >= 10 && index <= 19) {
      const newImageBackgrounds = [...imageBackgrounds];
      newImageBackgrounds[index] = 'green';
      correctAnswer();
      setpuan( puan => puan+1);
      setImageBackgrounds(newImageBackgrounds);
    } else if (option !== 'B' && index >= 10 && index <= 19) {
      const newImageBackgrounds = [...imageBackgrounds];
      newImageBackgrounds[index] = 'red';
      setpuan( puan => puan-1);
      wrongAnswer();
    ...
    if (option === 'C' && index >= 20 && index <= 28) {
      const newImageBackgrounds = [...imageBackgrounds];
      newImageBackgrounds[index] = 'green';
      correctAnswer();
      setpuan( puan => puan+1);
      setImageBackgrounds(newImageBackgrounds);
    } else if (option !== 'C' && index >= 20 && index <= 28) {
      const newImageBackgrounds = [...imageBackgrounds];
      newImageBackgrounds[index] = 'red';
      wrongAnswer();
      setpuan( puan => puan-1);
    ...
    if (option === 'D' && index >= 29 && index <= 39) {
      const newImageBackgrounds = [...imageBackgrounds];
      newImageBackgrounds[index] = 'green';
      correctAnswer();
      setpuan( puan => puan+1);
      setImageBackgrounds(newImageBackgrounds);
    } else if (option !== 'D' && index >= 29 && index <= 39) {
      const newImageBackgrounds = [...imageBackgrounds];
      newImageBackgrounds[index] = 'red';
      wrongAnswer();
      setpuan( puan => puan-1);
  ...
  const Counter = () => {
   ...
   return (
    <div style={{margin:"auto 1rem"}}>
      <div style={{margin:"auto 1rem"}} >
        <h2 style={{backgroundColor:"red", padding:"1rem" ,color:"white", margin:"auto"}} >7.SINIF TÜRKÇE SORULAR</h2>
        </div>
      
      <div style={{margin:"auto 1rem",border:"2px solid red"}} >
        <div className="  d-flex flex-column align-items-center  ">
          <div style={{padding:".5rem",border:"3px solid grey",width:"fit-content",margin:"1rem auto",borderRadius:"8px",boxShadow:"0px 0px 5px", backgroundColor: imageBackgrounds[randomIndex] }}>
            <img  src={`/images/turkce7images/image${randomIndex + 1}.png`} alt={`Random Image`} />
            <div >
          ...
          <Counter  />
          <p style={{color:"white",fontSize:"2rem", marginLeft:"auto", padding:"1rem",backgroundColor:"red"}}> Puan Durumu: {puan}</p>
          
          {((count<=0) )&&<p style={{color:"white",fontSize:"2rem", marginLeft:"auto", padding:"1rem",backgroundColor:"red"}}> {puan} puan kazandın. Tebrikler !</p>}
  
          {((limitedQues<6) && (count>0) )&&( <button style={{backgroundColor:"red",color:"white",padding:".5rem"}}   onClick={handleClick}>SONRAKİ SORU</button>)}
        </div>
        <button onClick={() => window.location.reload(false)} style={{backgroundColor:"red",color:"white",padding:".5rem"}} >SAYFAYI YENİLE</button>
  </div>
    </div>
  );
}

export default Soru7turkce;
